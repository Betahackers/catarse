...
def generate_project(fields = {})  
   p = Project.create!({ user: User.where(email: 'org@org.com').first,
                     ...
                     permalink: "foo_page_#{rand(9999999)}",
                     goal: [40000, 73000, 1000, 50000, 100000].shuffle.first,
                     online_date: Time.now,
                     online_days: [5, 60, 43, 14, 34].shuffle.first,
                     more_links: 'Ipsum dolor',
                     first_contributions: 'Foo bar',
                     # state: 'online',
   ...
   [3, 5, 7].shuffle.first.times { Reward.create! project: p, minimum_value: [10, 20, 30, 40, 50, 60, 70].shuffle.first, deliver_at: 10.days.from_now, description: Faker::Lorem.paragraph(2) }
   p
end
...
puts 'Creating channel...'
  ...
  c = Channel.create! name: 'Channel Name',
                      permalink: 'channel' + rand(999).to_s,
...
puts 'Creating channel projects...... It can take a while... You can go and get a coffee now!'
  ...
  channel_project.send_to_analysis!
...
puts 'Creating failed projects...... It can take a while...'
  ...
  2.times do
    p = generate_project(state: 'online', goal: 100000)
    [4, 7, 15, 30].shuffle.first.times { generate_contribution(p) }
...
puts 'Creating online projects...... It can take a while...'
  ...
  p = Project.last
  p.update_column(:recommended, true)
...
puts 'Creating soon projects ...... It can take a while...'
  ...
  4.times do
    generate_project(state: 'waiting_funds')
  end
